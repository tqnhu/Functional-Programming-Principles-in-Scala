...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val group_means = points.groupBy(p => findClosest(p, means))
    means.map(m => m -> group_means.getOrElse(m, GenSeq())).toMap
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val map_means = classify(points, means)
    val new_means = update(map_means, means)
    if ((converged(eta)(means, new_means))== false)
      kMeans(points, new_means, eta)
    else new_means
  }
}
