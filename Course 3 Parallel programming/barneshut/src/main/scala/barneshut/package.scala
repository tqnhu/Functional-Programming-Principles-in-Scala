...
package object barneshut {
  ...
  extends Quad {
    ...
    val massX = bodies.foldLeft(0.0f)((v,b)=>v+b.mass*b.x)/mass
    val massY = bodies.foldLeft(0.0f)((v,b)=>v+b.mass*b.y)/mass
    val total: Int = bodies.size
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        val quad = new Fork(Empty(centerX - size/4, centerY - size/4, size/2),
                            Empty(centerX + size/4, centerY - size/4, size/2),
                            Empty(centerX - size/4, centerY + size/4, size/2),
                            Empty(centerX + size/4, centerY + size/4 ,size/2))
        (bodies :+ b).foreach(quad.insert)
        quad
      }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x = Math.floor((b.x - boundaries.minX)/sectorSize).toInt max 0 min sectorPrecision - 1
      val y = Math.floor((b.y - boundaries.minY)/sectorSize).toInt max 0 min sectorPrecision - 1
      apply(x, y)+=b
      this
    }
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      val newMatrix = this.matrix.zip(that.matrix).par.map(p => p._1 combine p._2).toArray
      for (i <- 0 until matrix.length) matrix.par.update(i, newMatrix(i))
      this
    }
